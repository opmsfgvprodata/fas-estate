...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class CheckRollFGVController : Controller
    {
        ...
        public ActionResult WorkingDetails()
        {
            ...
            Division = new SelectList(db.vw_NSWL_2.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).OrderBy(o => o.fld_DivisionName).Select(s => new SelectListItem { Value = s.fld_DivisionID.ToString(), Text = s.fld_DivisionName }), "Value", "Text", DivisionID).ToList();
            ...
            //Shah 23-05-2024
            var CheckLastDataKeyIn = dbr.tbl_Kerja.Join(dbr.tbl_Pkjmast, j => new { j.fld_Nopkj, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID }, k => new { k.fld_Nopkj, k.fld_NegaraID, k.fld_SyarikatID, k.fld_WilayahID, k.fld_LadangID }, (j, k) => new { j.fld_Tarikh, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID, j.fld_Nopkj }).Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Tarikh.Value.Year == date.Value.Year && x.fld_Tarikh.Value.Month == date.Value.Month).OrderByDescending(o => o.fld_Tarikh).Select(s => s.fld_Tarikh).FirstOrDefault();

            if (CheckLastDataKeyIn == null)
            {
                CheckLastDataKeyIn = new DateTime(Today.Year, Today.Month, 1);
            }

            var isCheckrollBlock = EstateFunction.isCheckrollBlock(timezone.gettimezone(), CheckLastDataKeyIn.Value, int.Parse(CheckBlockKeyInDay), NegaraID, SyarikatID, WilayahID, LadangID);

            if (isCheckrollBlock)
            {
                dbr.Dispose();
                return RedirectToAction("CheckRollBlock", "CheckRollFGV", new { msg = 1 });
            }
            ...
            //    return RedirectToAction("CheckRollBlock", "CheckRollFGV", new { msg = 1 });
            //}
            /*else if (TotalDayLastKeyIn <= TotalDayLastNeedKeyIn && CheckBlockStatus != null)  //fitri comment 10-08-2020
            {
                if (CheckBlockStatus.fld_BlokStatus == true)
                {
                    ApprovalDayCount = (Today - CheckBlockStatus.fld_ValidDT).Value.TotalDays;
            ...
            //else if (TotalDayLastKeyIn <= TotalDayLastNeedKeyIn && CheckBlockStatus != null) //fitri add 10-08-2020
            //{
            //    if (CheckBlockStatus.fld_BlokStatus == true)
            //    {
            //        dbr.Dispose();
        ...
        [HttpPost]
        public ActionResult Attendance(CustMod_Attandance CustMod_Attandance, string[] SaveWorkerNo)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = 0;
            int? DivisionID2 = CustMod_Attandance.Division;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
            string msg = "";
            string statusmsg = "";
            ...
            try
            {
                DateTime timeIn = DateTime.ParseExact(CustMod_Attandance.TimeIn, "HH:mm", CultureInfo.CurrentCulture);
                DateTime timeOut = DateTime.ParseExact(CustMod_Attandance.TimeOut, "HH:mm", CultureInfo.CurrentCulture);
                TimeSpan totalTimeSpan = (timeOut - timeIn).Duration();
                decimal totalWorkingHour = totalTimeSpan.Hours;
                if (totalTimeSpan.Minutes >= 30)
                {
                    totalWorkingHour = totalWorkingHour + 0.5m;
                }
                ...
                {
                    var totalDaysWorking = int.Parse(db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "totaldaysworking" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault());
                    var lastMonth = CustMod_Attandance.dateseleted.AddMonths(-1);

                    CutOfDateStatus = EstateFunction.GetStatusCutProcess(dbr, CustMod_Attandance.dateseleted, NegaraID, SyarikatID, WilayahID, LadangID, DivisionID);
                    if (!CutOfDateStatus)
                    {
                        ...
                        {
                            KumpulanID = dbr.tbl_KumpulanKerja.Where(x => x.fld_KodKumpulan.Trim() == CustMod_Attandance.SelectionData.Trim() && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).Select(s => s.fld_KumpulanID).FirstOrDefault();
                            var pkjids = dbr.tbl_Pkjmast.Where(x => x.fld_KumpulanID == KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).Select(s => s.fld_Nopkj.Trim()).ToList();
                            var tbl_Kerjahdr = dbr.tbl_Kerjahdr.Where(x => pkjids.Contains(x.fld_Nopkj) && ((x.fld_Tarikh.Value.Year == CustMod_Attandance.dateseleted.Year && x.fld_Tarikh.Value.Month == CustMod_Attandance.dateseleted.Month) || (x.fld_Tarikh.Value.Year == lastMonth.Year && x.fld_Tarikh.Value.Month == lastMonth.Month)) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).ToList();
                            var datainkrjhdrs = tbl_Kerjahdr.Where(x => pkjids.Contains(x.fld_Nopkj) && x.fld_Tarikh == CustMod_Attandance.dateseleted && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).ToList(); // Baru Edit
                            if (datainkrjhdrs.Count() == 0)
                            {
                                foreach (var pkjid in pkjids)
                                {
                                    if (SaveWorkerNo.Contains(pkjid))
                                    {
                                        if (CustMod_Attandance.WorkCode == "H02")
                                        {
                                            var dateToCheckWorking = CustMod_Attandance.dateseleted.AddDays(-totalDaysWorking);
                                            isHaveH02 = tbl_Kerjahdr.Any(x => x.fld_Kdhdct == "H02" && x.fld_Nopkj == pkjid && x.fld_Tarikh >= dateToCheckWorking && x.fld_Tarikh <= CustMod_Attandance.dateseleted);
                                        }
                                        if (LeaveSelection)
                                        {
                        ...
                        {
                            var tbl_Kerjahdr = dbr.tbl_Kerjahdr.Where(x => x.fld_Nopkj.Trim() == CustMod_Attandance.SelectionData && ((x.fld_Tarikh.Value.Year == CustMod_Attandance.dateseleted.Year && x.fld_Tarikh.Value.Month == CustMod_Attandance.dateseleted.Month) || (x.fld_Tarikh.Value.Year == lastMonth.Year && x.fld_Tarikh.Value.Month == lastMonth.Month)) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).ToList();
                            var datainkrjhdr = tbl_Kerjahdr.Where(x => x.fld_Nopkj.Trim() == CustMod_Attandance.SelectionData && x.fld_Tarikh == CustMod_Attandance.dateseleted && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).FirstOrDefault();
                            ...
                            if (datainkrjhdr == null)
                            {
                                ...
                                {
                                    var dateToCheckWorking = CustMod_Attandance.dateseleted.AddDays(-totalDaysWorking);
                                    isHaveH02 = tbl_Kerjahdr.Any(x => x.fld_Kdhdct == "H02" && x.fld_Nopkj == CustMod_Attandance.SelectionData && x.fld_Tarikh >= dateToCheckWorking && x.fld_Tarikh <= CustMod_Attandance.dateseleted);
                                }
                                if (LeaveSelection)
                                {
                                    ...
                                    {
                                        var pkjdata = dbr.tbl_Pkjmast.Where(x => x.fld_Nopkj == CustMod_Attandance.SelectionData && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).Select(s => new { s.fld_Nopkj, s.fld_KumpulanID }).FirstOrDefault();
                                        KumpulanKod = dbr.tbl_KumpulanKerja.Where(x => x.fld_KumpulanID == pkjdata.fld_KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).Select(s => s.fld_KodKumpulan).FirstOrDefault();
                        ...
                        if (tbl_Kerjahdrs.Count() != 0 && !isHaveH02)
                        {
                        ...
                        if (isHaveH02)
                        {
                            msg = "The worker has a record of working consecutive weekends within 14 days.";
                            statusmsg = "warning";
                        }
                    }
                    else
                ...
                var isActionLocked = EstateFunction.isActionLocked(CustMod_Attandance.dateseleted, timezone.gettimezone(), dackDatedDay, NegaraID, SyarikatID, WilayahID, LadangID);

                if (CustMod_Attandance.SelectionCategory == 1)
                {
                    var pkjids = dbr.tbl_Pkjmast.Where(x => x.fld_KumpulanID == KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).Select(s => s.fld_Nopkj.Trim()).ToList(); //Baru Edit
            ...
            if (string.IsNullOrEmpty(idpengurus) == false && string.IsNullOrEmpty(passpengurus) == false)
            {
                ...
                if (pengurus != null)
                {
                    try
                    {
                        ...
                        {
                            CutOfDateStatus = EstateFunction.GetStatusCutProcess(dbr, CustMod_HariTerabai.dateseleted, NegaraID, SyarikatID, WilayahID, LadangID, DivisionID);
                            if (!CutOfDateStatus)
                            {
                                ...
                                if (tbl_Kerjahdrs.Count() != 0)
                                {
                                    ...
                                    EstateFunction.SaveDataKerjaSAP(dbr, tbl_Kerjas, HadirData, NegaraID, SyarikatID, WilayahID, LadangID, GLCode, CustMod_HariTerabai.NNCCHT, CustMod_HariTerabai.SAPPilihanAktvtHT);
                                }
                            }
                            else
                            {
                        ...
                        var dackDatedDay = int.Parse(db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "backdatedkeyin" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault());
                        var isActionLocked = EstateFunction.isActionLocked(CustMod_HariTerabai.dateseleted, timezone.gettimezone(), dackDatedDay, NegaraID, SyarikatID, WilayahID, LadangID);
        ...
        public ActionResult WorkDetail()
        {
            ...
            TrnsfrLvl.Add(new SelectListItem { Text = "No", Value = "0", Selected = true });
            TrnsfrLvl.Add(new SelectListItem { Text = "Yes", Value = "1", Selected = false });
        ...
        public ActionResult Working(DateTime SelectDate, string SelectionData, int SelectionCategory, string Lejar, byte JnisPkt, string PilihanPkt, string JnisAktvt, string PilihanAktvt, string NNCC, string SelectionNNCC, string SAPPilihanAktvt, decimal? HrgaKwsnSkr, string KdKwsnSkr, int AppKwnsSkrLainID, DateTime? AppKwnsSkrLainDT, int Division, byte TrnsfrLvl, List<CustMod_Work> HadirData)
        {
            ...
            int? DivisionID = 0;
            int? DivisionID2 = Division;
            int KumpulanID = 0;
            List<tbl_Kerja> tbl_KerjaList = new List<tbl_Kerja>();
            string KodInsentif = "";
            decimal? ValInsentif = 0;
            List<string> pkjids = null;
            List<tbl_Kerja> ExistingKerja = new List<tbl_Kerja>();
            bool TransferPkt = false;
            List<tbl_Kerja> monthlyDataWorking = new List<tbl_Kerja>();
            ...
            if (HadirData != null)
            {
                ...
                { //Baru Edit
                    if (SelectionNNCC == "1")
                    {
                        ...
                        {
                            KumpulanID = dbr.tbl_KumpulanKerja.Where(x => x.fld_KodKumpulan.Trim() == SelectionData.Trim() && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).Select(s => s.fld_KumpulanID).FirstOrDefault();
                            pkjids = dbr.tbl_Pkjmast.Where(x => x.fld_KumpulanID == KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).Select(s => s.fld_Nopkj.Trim()).ToList();
                    ...
                    var GetStatusPieceRateKongKeyIn = db.tblOptionConfigsWebs.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldOptConfFlag1 == "allwpceratekong" && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault();
                    checkkongactvt = GetStatusPieceRateKongKeyIn == "True" ? 0 : checkkongactvt;
                    ...
                    if (pkjids != null)
                    {
                        monthlyDataWorking = dbr.tbl_Kerja.Where(x => x.fld_Tarikh.Value.Month == SelectDate.Month && x.fld_Tarikh.Value.Year == SelectDate.Year && pkjids.Contains(x.fld_Nopkj)).ToList();
                        ExistingKerja = monthlyDataWorking.Where(x => x.fld_Tarikh == SelectDate && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && pkjids.Contains(x.fld_Nopkj) && x.fld_KodPkt == PilihanPkt && x.fld_KodAktvt == PilihanAktvt).ToList();
                    }
                    else
                    {
                        monthlyDataWorking = dbr.tbl_Kerja.Where(x => x.fld_Tarikh.Value.Month == SelectDate.Month && x.fld_Tarikh.Value.Year == SelectDate.Year && x.fld_Nopkj == SelectionData).ToList();
                        ExistingKerja = monthlyDataWorking.Where(x => x.fld_Tarikh == SelectDate && x.fld_Nopkj == SelectionData && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_KodPkt == PilihanPkt && x.fld_KodAktvt == PilihanAktvt).ToList();
                    }

                    if (HadirData.Count() != 0 && checkkongactvt == 0)
                    {
                        string[] otMaxHours = new string[] { "maxdailyothours", "maxmonthlyothours" };
                        var blckOTHours = db.tblOptionConfigsWebs.Where(x => otMaxHours.Contains(x.fldOptConfFlag1) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).ToList();
                        var maxDailyOTHours = int.Parse(blckOTHours.Where(x => x.fldOptConfFlag1 == "maxdailyothours").Select(s => s.fldOptConfValue).FirstOrDefault());
                        var maxMonthlyOTHours = int.Parse(blckOTHours.Where(x => x.fldOptConfFlag1 == "maxmonthlyothours").Select(s => s.fldOptConfValue).FirstOrDefault());

                        var isOTExceed = false;

                        foreach (var datakerja in HadirData)
                        {
                            var currentOT = datakerja.ot;
                            var existingOT = monthlyDataWorking.Where(x => x.fld_Nopkj == datakerja.nopkj && x.fld_Tarikh == SelectDate).Sum(s => s.fld_JamOT);
                            var todayOT = currentOT + existingOT;
                            var monthlyOT = monthlyDataWorking.Where(x => x.fld_Nopkj == datakerja.nopkj).Sum(s => s.fld_JamOT) + currentOT;

                            if (todayOT > maxDailyOTHours || monthlyOT > maxMonthlyOTHours)
                            {
                                isOTExceed = true;
                            }
                            ...
                            if (datakerja.hasil > 0)
                            {
                                ...
                                datakerja.ot = datakerja.ot == null ? 0 : datakerja.ot;
                                ...
                                unitcode = db.tbl_UpahAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodAktvt == PilihanAktvt && x.fld_Deleted == false).Select(s => s.fld_Unit).FirstOrDefault();
                                ...
                                if (datakerja.incentiveval > 0)
                                {
                                    ...
                                    if (datakerja.incentiveval == null || datakerja.incentiveval == 0 || datakerja.incentivecode == "N/A")
                                    {
                                        KodInsentif = "N/A";
                                        ValInsentif = 0;
                                    }
                                    ...
                                    {
                                        KodInsentif = datakerja.incentivecode;
                                        ValInsentif = datakerja.incentiveval;
                                    }
                                    DailyIncentiveCode = KodInsentif;
                                    DailyIncentive = ValInsentif;
                                    //}
                                }
                                else
                                {
                        ...
                        else
                        {
                            if (isOTExceed)
                            {
                                msg = "OT hours exceed the limit.";
                                statusmsg = "warning";
                            }
                            else
                            {
                                msg = GlobalResEstate.msgDataExist + " or no data to be saved";
                                statusmsg = "warning";
                            }
                        }
                    }
        ...
        public JsonResult DeleteAttInfo(Guid Data, int SelectionCategory, string SelectionData, DateTime SelectDate, int Division)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = Division;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
            string msg = "";
            string statusmsg = "";
            ...
            var backDatedDay = int.Parse(db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "backdatedkeyin" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault());
            var isActionLocked = EstateFunction.isActionLocked(SelectDate, timezone.gettimezone(), backDatedDay, NegaraID, SyarikatID, WilayahID, LadangID);
            ...
            if (SelectionCategory == 1)
            {
                ...
                if (datainkrjhdr.Count() > 0)
                {
                    ...
                    foreach (var pkjmast1 in pkjmasts1)
                    {
                        ...
                        CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = pkjmast1.fld_Nopkj, fld_Nama = pkjmast1.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Status = "Tiada rekod", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = "-", fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_LadangID = LadangID, isActionLocked = isActionLocked });
            ...
            else
            {
                ...
                {
                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = datainpkjmast.fld_Nopkj, fld_Nama = datainpkjmast.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Tarikh = SelectDate, fld_Status = "Tiada rekod", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = "-", fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_LadangID = LadangID, isActionLocked = isActionLocked });
        ...
        public ActionResult DeleteWorkInfo(DateTime SelectDate, string SelectionData, int SelectionCategory, string pkt, string kodatvt, int Division)
        {
            //Edit Baru
            string msg = "";
            string statusmsg = "";
        ...
        public JsonResult WorkerData(int SelectionCategory, int Division)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
        ...
        public JsonResult WorkerAvlbleChecking(int SelectionCategory, string SelectionData, int Division)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = Division;
            int? getuserid = getidentity.ID(User.Identity.Name);
        ...
        public JsonResult WorkerDateChecking(int SelectionCategory, string SelectionData, DateTime SelectDate, int Division)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = 0;
            int? DivisionID2 = Division;
            ...
            var dackDatedDay = int.Parse(db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "backdatedkeyin" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault());
            var isActionLocked = EstateFunction.isActionLocked(SelectDate, timezone.gettimezone(), dackDatedDay, NegaraID, SyarikatID, WilayahID, LadangID);

            if (SelectionCategory == 1)
            {
                //check kehadiran
                KumpulanID = dbr.tbl_KumpulanKerja.Where(x => x.fld_KodKumpulan.Trim() == SelectionData.Trim() && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).Select(s => s.fld_KumpulanID).FirstOrDefault();
                var datainpkjmast = dbr.tbl_Pkjmast.Where(x => x.fld_KumpulanID == KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).Select(s => s.fld_Nopkj.Trim()).ToList();
                ...
                if (datainkrjhdr.Count() > 0)
                {
                    ...
                    var pkjmasts1 = dbr.tbl_Pkjmast.Where(x => datainpkjmastexldatainkrjhdrs.Contains(x.fld_Nopkj) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).OrderBy(o => o.fld_Nopkj).ToList();

                    var pkjmasts2 = dbr.tbl_Pkjmast.Where(x => datainkrjhdr.Contains(x.fld_Nopkj) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).OrderBy(o => o.fld_Nopkj).ToList();

                    foreach (var pkjmast1 in pkjmasts1)
                    {
                        tbl_KumpulanKerja = dbr.tbl_KumpulanKerja.Where(x => x.fld_KumpulanID == pkjmast1.fld_KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).FirstOrDefault();
                    ...
                    foreach (var pkjmast2 in pkjmasts2)
                    {
                        ...
                        tbl_KumpulanKerja = dbr.tbl_KumpulanKerja.Where(x => x.fld_KumpulanID == pkjmast2.fld_KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).FirstOrDefault(); //Baru Edit
                ...
                {
                    var pkjmasts1 = dbr.tbl_Pkjmast.Where(x => datainpkjmast.Contains(x.fld_Nopkj) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).OrderBy(o => o.fld_Nopkj).ToList();
                    foreach (var pkjmast1 in pkjmasts1)
                    {
                        tbl_KumpulanKerja = dbr.tbl_KumpulanKerja.Where(x => x.fld_KumpulanID == pkjmast1.fld_KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).FirstOrDefault();
                        GajiTerkumpul = dbr.tbl_Kerja.Where(x => x.fld_Nopkj == pkjmast1.fld_Nopkj && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Tarikh.Value.Month == SelectDate.Month && x.fld_Tarikh.Value.Year == SelectDate.Year).Sum(s => s.fld_OverallAmount); //Baru Edit
            ...
            {
                var datainpkjmast = dbr.tbl_Pkjmast.Where(x => x.fld_Nopkj == SelectionData && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).FirstOrDefault();
                ...
                //var datainkrjhdr = dbr.tbl_Kerjahdr.Where(x => x.fld_Nopkj.Trim() == SelectionData && x.fld_Tarikh == SelectDate && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).FirstOrDefault();
                tbl_KumpulanKerja = dbr.tbl_KumpulanKerja.Where(x => x.fld_KumpulanID == datainpkjmast.fld_KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).FirstOrDefault();
                if (datainkrjhdr != null)
                {
                ...
                else
                {
                    ...
                    GajiTerkumpul = GajiTerkumpul == null ? 0 : GajiTerkumpul;
                    CustMod_Kerjahdrs.Add(new CustMod_Kerjahdr() { fld_Nopkj = datainpkjmast.fld_Nopkj, fld_Nama = datainpkjmast.fld_Nama, fld_Kum = tbl_KumpulanKerja.fld_KodKumpulan, fld_Tarikh = SelectDate, fld_Status = "Tiada rekod", fld_HdrCt = "-", fld_Hujan = "-", fld_GajiTerkumpul = GajiTerkumpul.ToString(), fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_LadangID = LadangID, isActionLocked = isActionLocked });
                    namelabel = datainpkjmast.fld_Nama;
                    msg = GlobalResEstate.msgNoRecord;
                    statusmsg = "success";
                    disablesavebtn = false;
            ...
            {
                namepkj = EstateFunction.PkjName(dbr, NegaraID, SyarikatID, WilayahID, LadangID, tbl_KerjaData.fld_Nopkj);
                CustMod_WorkerWorks.Add(new CustMod_WorkerWork() { fld_ID = tbl_KerjaData.fld_ID, fld_Nopkj = tbl_KerjaData.fld_Nopkj, fld_NamaPkj = namepkj, fld_Amount = tbl_KerjaData.fld_Amount, fld_JumlahHasil = tbl_KerjaData.fld_JumlahHasil, fld_KodAktvt = tbl_KerjaData.fld_KodAktvt, fld_KodGL = tbl_KerjaData.fld_KodGL, fld_KodPkt = tbl_KerjaData.fld_KodPkt, fld_Kum = tbl_KerjaData.fld_Kum, fld_Tarikh = tbl_KerjaData.fld_Tarikh, fld_JamOT = tbl_KerjaData.fld_JamOT, fld_Unit = tbl_KerjaData.fld_Unit, fld_NegaraID = NegaraID, fld_SyarikatID = SyarikatID, fld_WilayahID = WilayahID, fld_LadangID = LadangID, fld_AmountOA = tbl_KerjaData.fld_OverallAmount, fld_DailyIncentive = tbl_KerjaData.fld_DailyIncentive, isActionLocked = isActionLocked });
            }
        ...
        public JsonResult GetAttandanceDetails(int SelectionCategory, string SelectionData, int Division)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = Division;
            int? getuserid = getidentity.ID(User.Identity.Name);
        ...
        public JsonResult GetNNCC_V2(string JnisAktvt, byte JnsPkt, string PilihanPkt, byte TrnsfrLvl)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            if (TrnsfrLvl == 0)
            {
                ...
                if (JnisAktvt == "E")
                {
                    ...
                    var AktivitiNNCCMap = db.tbl_KategoriAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_Deleted == false && x.fld_KodJnsAktvt == JnisAktvt).ToList();
                    ...
                    NNCC = new SelectList(AktivitiNNCCMap.OrderBy(o => o.fld_Kategori).Select(s => new SelectListItem { Value = s.fld_KodKategori, Text = s.fld_Kategori }), "Value", "Text").ToList();
                    NNCC.Insert(0, (new SelectListItem { Text = GlobalResEstate.lblChoose, Value = "0" }));
                    SAPKodStatus = "1";
                ...
                else
                {
                    ...
                    NNCC = new SelectList(ListCC.OrderBy(o => o.fld_CostCenter).Select(s => new SelectListItem { Value = s.fld_CostCenter, Text = s.fld_CostCenter + " - " + s.fld_CostCenterDesc }), "Value", "Text").ToList();
                    //NNCC.Insert(0, (new SelectListItem { Text = GlobalResEstate.lblChoose, Value = "0" }));
                    SAPKodStatus = "2";
                    //var GetNNCC = NNCCList.Select(s => s.fld_NNCC).Take(1).FirstOrDefault();
                    var KodActvtList = db.tbl_UpahAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodJenisAktvt == JnisAktvt && x.fld_Deleted == false).ToList();
                    KodActvt = new SelectList(KodActvtList.OrderBy(o => o.fld_KodAktvt).Select(s => new SelectListItem { Value = s.fld_KodAktvt, Text = s.fld_KodAktvt + " - " + s.fld_Desc }), "Value", "Text").ToList();
            ...
            {
                var SelectPkt4 = dbr.tbl_PktPinjam.Where(x => x.fld_KodPkt == PilihanPkt && x.fld_JenisPkt == JnsPkt && x.fld_JnsLot == JnisAktvt && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).FirstOrDefault();
                SapDataPkt = SelectPkt4.fld_SAPCode;
                ...
                {
                    var ListCC = db.tbl_SAPCCPUP.Where(x => x.fld_NegaraID == SelectPkt4.fld_NegaraIDAsal && x.fld_SyarikatID == SelectPkt4.fld_SyarikatIDAsal && x.fld_WilayahID == SelectPkt4.fld_WilayahIDAsal && x.fld_LadangID == SelectPkt4.fld_LadangIDAsal && x.fld_CostCenter == SapDataPkt).ToList();
                    //var AktivitiNNCCMap = db.tbl_AktivitiNNCCMap.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Deleted == false && x.fld_JenisAktiviti == JnisAktvt).ToList();
                    //ListNNCC = ListCC.Select(s => s.fld_CostCenter).Distinct().ToList();
                    //var NNCCList = AktivitiNNCCMap.Join(ListCC, j => new { fld_NNCC = j.fld_NNCC }, k => new { fld_NNCC = k.fld_CostCenter }, (j, k) => new { j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID, j.fld_JenisAktiviti, j.fld_Deleted, j.fld_NNCC, j.fld_Flag, k.fld_CostCenterDesc }).Where(x => x.fld_JenisAktiviti == JnisAktvt && ListNNCC.Contains(x.fld_NNCC)).Select(s => new { s.fld_NNCC, s.fld_Flag, s.fld_CostCenterDesc }).Distinct().ToList(); //db.tbl_AktivitiNNCCMap.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Deleted == false && x.fld_JenisAktiviti == JnisAktvt && ListNNCC.Contains(x.fld_NNCC)).Select(s => new { s.fld_NNCC, s.fld_Flag }).Distinct().ToList();
                    NNCC = new SelectList(ListCC.OrderBy(o => o.fld_CostCenter).Select(s => new SelectListItem { Value = s.fld_CostCenter, Text = s.fld_CostCenter + " - " + s.fld_CostCenterDesc }), "Value", "Text").ToList();
        ...
        public JsonResult GetNNCC(string JnisAktvt, byte JnsPkt, string PilihanPkt, byte TrnsfrLvl)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            else
            {
                ...
                SAPKodStatus = NNCCList.Select(s => s.fld_Flag).Take(1).FirstOrDefault();
                var GetNNCC = NNCCList.Select(s => s.fld_NNCC).Take(1).FirstOrDefault();
                var KodActvtList = db.tbl_AktivitiNNCCMap.Join(db.tbl_UpahAktiviti, j => new { j.fld_KodAktivitiOPMS, j.fld_NegaraID, j.fld_SyarikatID }, k => new { fld_KodAktivitiOPMS = k.fld_KodAktvt, k.fld_NegaraID, k.fld_SyarikatID }, (j, k) => new { j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID, k.fld_KodAktvt, k.fld_DisabledFlag, j.fld_Deleted, k.fld_Desc, j.fld_KodAktivitiOPMS, j.fld_KodAktivitiSAP, j.fld_JenisAktiviti, j.fld_NNCC }).Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_LadangID == LadangID && x.fld_Deleted == false && x.fld_JenisAktiviti == JnisAktvt && x.fld_NNCC == GetNNCC).ToList();
        ...
        public JsonResult GetPktTrnsfr(string JnisAktvt, byte JnsPkt, byte TrnsfrLvl)
        {
            ...
            var SelectPkt = dbr.tbl_PktPinjam.Where(x => x.fld_JnsLot == JnisAktvt && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_JenisPkt == JnsPkt && x.fld_EndDT >= CurrentDate).ToList();
        ...
        public JsonResult GetAktvt_V2(string NNCCSelection, string JnisAktvt, string SelectionNNCC, string SAPPilihanAktvt, int Purpose, int HTPurpose)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);
            ...
            if (Purpose == 1)
            {
                if (HTPurpose == 1)
                {
                    var KodActvtList = db.tbl_UpahAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KategoriAktvt == NNCCSelection && x.fld_DisabledFlag == 3 && x.fld_Deleted == false).ToList();
                    if (SelectionNNCC == "1")
                    {
                        KodActvt = new SelectList(KodActvtList.OrderBy(o => o.fld_KodAktvt).Select(s => new SelectListItem { Value = s.fld_KodAktvt, Text = s.fld_KodAktvt + " - " + s.fld_Desc }), "Value", "Text").ToList();
                    }
                    ...
                    {
                        KodActvt = new SelectList(KodActvtList.OrderBy(o => o.fld_KodAktvt).Select(s => new SelectListItem { Value = s.fld_KodAktvt, Text = s.fld_KodAktvt + " - " + s.fld_Desc }), "Value", "Text").ToList();
                        var KodActvtDesc = KodActvtList.Select(s => s.fld_KodAktvt).ToArray();
                        AktivitiToolTip = db.tbl_UpahAktiviti.Where(x => KodActvtDesc.Contains(x.fld_KodAktvt) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).OrderBy(o => o.fld_KodAktvt).Select(s => new { Label = s.fld_KodAktvt + " - " + s.fld_Desc + " - RM" + s.fld_Harga }).ToList();
                    }
                }
                else
                {
                    var KodActvtList = db.tbl_UpahAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KategoriAktvt == NNCCSelection && x.fld_Deleted == false).ToList();
                    if (SelectionNNCC == "1")
                    {
                        KodActvt = new SelectList(KodActvtList.OrderBy(o => o.fld_KodAktvt).Select(s => new SelectListItem { Value = s.fld_KodAktvt, Text = s.fld_KodAktvt + " - " + s.fld_Desc }), "Value", "Text").ToList();
                    }
            ...
            else
            {
                ...
                {
                    var KodActvtList = db.tbl_UpahAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KategoriAktvt == NNCCSelection).ToList();
                    KodActvt = new SelectList(KodActvtList.OrderBy(o => o.fld_KodAktvt).Select(s => new SelectListItem { Value = s.fld_KodAktvt, Text = s.fld_KodAktvt + " - " + s.fld_Desc }), "Value", "Text").ToList();
                    var KodActvtDesc = KodActvtList.Select(s => s.fld_KodAktvt).ToArray();
                    AktivitiToolTip = db.tbl_UpahAktiviti.Where(x => KodActvtDesc.Contains(x.fld_KodAktvt) && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID).OrderBy(o => o.fld_KodAktvt).Select(s => new { Label = s.fld_KodAktvt + " - " + s.fld_Desc + " - RM" + s.fld_Harga }).ToList();
                }
        ...
        public JsonResult GetGLSAP(byte JnisPkt, string PilihanPkt, string PilihanAktvt)
        {
            ...
            var tbl_JenisAktiviti = db.tbl_UpahAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodAktvt == PilihanAktvt).FirstOrDefault();
            ...
            if (EstateFunction.CheckSAPGLMap(dbr, JnisPkt, PilihanPkt, KodAktvt, NegaraID, SyarikatID, WilayahID, LadangID, false, "-", out GLCode))
            {
                ...
                if (kadarharga != 0)
                {
                    if (SelectionCategory == 1)
                    {
                        ...
                        var checkatts = dbr.tbl_Kerjahdr.Join(dbr.tbl_Pkjmast, j => new { j.fld_Nopkj, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID }, k => new { k.fld_Nopkj, k.fld_NegaraID, k.fld_SyarikatID, k.fld_WilayahID, k.fld_LadangID }, (j, k) => new { j.fld_Kum, j.fld_Tarikh, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID, k.fld_Nama, j.fld_Nopkj, j.fld_Kdhdct, k.fld_DivisionID }).Where(x => datainpkjmast.Contains(x.fld_Nopkj) && x.fld_Tarikh == SelectDate && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_DivisionID == DivisionID).ToList();
        ...
        public JsonResult GetForm_V2(int SelectionCategory, string SelectionData, byte JnisPkt, string PilihanPkt, string JnisAktvt, string KodAktvt, DateTime SelectDate, string NNCC, int Division)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = 0;
            int? DivisionID2 = Division;
            ...
            if (JnisAktvt == "E")
            {
                CustMod_NNActCode = EstateFunction.GetNNActCode(dbr, JnisPkt, PilihanPkt, KodAktvt, NNCC, NegaraID, SyarikatID, WilayahID, LadangID, DivisionID, 0);
                if (CustMod_NNActCode != null)
                {
                    Proceed = true;
                    NetworkNoSAP = CustMod_NNActCode.NetworkNoSAP;
                    ActvtCodeSAP = CustMod_NNActCode.ActvtCodeSAP;
                }
                else
                {
                    Proceed = false;
            ...
            if (Proceed)
            {
                if (EstateFunction.CheckSAPGLMap(dbr, JnisPkt, PilihanPkt, KodAktvt, NegaraID, SyarikatID, WilayahID, LadangID, false, "-", out GLCode))
                {
                    ...
                    if (kadarharga != 0)
                    {
                        ...
                        {
                            var checkatt = dbr.tbl_Kerjahdr.Join(dbr.tbl_Pkjmast, j => new { j.fld_Nopkj, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID }, k => new { k.fld_Nopkj, k.fld_NegaraID, k.fld_SyarikatID, k.fld_WilayahID, k.fld_LadangID }, (j, k) => new { j.fld_Kum, j.fld_Tarikh, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID, k.fld_Nama, j.fld_Nopkj, j.fld_Kdhdct, k.fld_DivisionID }).Where(x => x.fld_Nopkj == SelectionData && x.fld_Tarikh == SelectDate && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_DivisionID == DivisionID2).FirstOrDefault();
                            var SalaryIncrement = dbr.tbl_PkjIncrmntSalary.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Nopkj == checkatt.fld_Nopkj && x.fld_AppStatus == true).Select(s => new { s.fld_IncrmntSalary, s.fld_DailyInsentif }).FirstOrDefault();
                            decimal? DailyInsentif = 0;
                            if (SalaryIncrement != null)
                            {
                    ...
                    var starttime = "16:00";
                    var endtime = "16:00";
                    var todaydt = EstateFunction.GetDateTime().Date.AddHours(16);
                    var tomorrowdt = EstateFunction.GetDateTime().Date.AddHours(22);
                    var timefilter = new List<timefilter>();
                    int id = 0;
                    for (DateTime i = todaydt; i < tomorrowdt; i = i.AddMinutes(timeinterval))
                    ...
                    //var JnisAktvtKod = db.tbl_UpahAktiviti.Where(x => x.fld_Deleted == false && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodJenisAktvt == JnisAktvt && x.fld_KodAktvt == KodAktvt).FirstOrDefault();
                    //JnisAktvtKod.fld_DisabledFlag - 1 = kong box & kualiti box xde, 2 = kong je xde, 3 = kong je kluar
                    bodyview = RenderRazorViewToString("WorkingDetailsForm", CustMod_AttWorkList.OrderBy(o => o.Namapkj), false, timeIn, timeOut);
                }
                else
                {
                    msg = "GL code not found.";
            ...
            {
                msg = "Network No SAP & Activity SAP not found.";
                statusmsg = "warning";
                closeform = true;
            }
            ...
            return Json(new { tablelisting = bodyview, msg, statusmsg, closeform, GLCode, NameAktiviti, NetworkNoSAP, ActvtCodeSAP });
        }

        public JsonResult GetForm_V3(int SelectionCategory, string SelectionData, byte JnisPkt, string PilihanPkt, string JnisAktvt, string KodAktvt, DateTime SelectDate, string NNCC, int Division, byte TrnsfrLvl)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = 0;
            int? DivisionID2 = Division;
            ...
            string ActvtGLSAP = "";
            int? KumpulanID = 0;
            decimal? GetLastMonthAverageSalary2 = 0;

            int? jenisbayaran = 0; //added by faeza 17.08.2022

            //added by faeza 17.08.2022
            //DateTime SelectDateOri = new DateTime(SelectDate.Year, SelectDate.Month, 15);
            DateTime LastSelectMonthDate = SelectDate.AddMonths(-1);
            ...
            if (JnisAktvt == "E")
            {
                CustMod_NNActCode = EstateFunction.GetNNActCode(dbr, JnisPkt, PilihanPkt, KodAktvt, NNCC, NegaraID, SyarikatID, WilayahID, LadangID, DivisionID, TrnsfrLvl);
                if (CustMod_NNActCode != null)
                {
                    Proceed = true;
                    NetworkNoSAP = CustMod_NNActCode.NetworkNoSAP;
            ...
            if (Proceed)
            {
                if (EstateFunction.CheckSAPGLMap(dbr, JnisPkt, PilihanPkt, KodAktvt, NegaraID, SyarikatID, WilayahID, LadangID, false, "-", out GLCode))
                {
                    var GetMapGL = db.tbl_MapGL.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).ToList();
                    var tbl_JenisAktiviti = db.tbl_UpahAktiviti.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodAktvt == KodAktvt && x.fld_Deleted == false).FirstOrDefault();
                    //tbl_JenisAktiviti.fld_DisabledFlag - 1 = kong box & kualiti box xde, 2 = kong je xde, 3 = kong je kluar
                    NameAktiviti = tbl_JenisAktiviti.fld_KodAktvt + " - " + tbl_JenisAktiviti.fld_Desc;
                    //modified by shah on 03.08.2020 - change from SpecialRateTable to SpecialRateTableIncldPnjmPkrj
                    kadarharga = tbl_JenisAktiviti.fld_KdhByr == "B" ? tbl_JenisAktiviti.fld_Harga : EstateFunction.SpecialRateTableIncldPnjmPkrja(SelectDate, JnisPkt, PilihanPkt, "A", dbr, NegaraID, SyarikatID, WilayahID, LadangID, out YieldBracketFullMonth, 2, TrnsfrLvl);
                    kadarharga2 = kadarharga;
                    //added by faeza 17.08.2022
                    jenisbayaran = tbl_JenisAktiviti.fld_DisabledFlag;
                    //end added
                    if (kadarharga != 0)
                    {
                        if (SelectionCategory == 1)
                        {
                            KumpulanID = dbr.tbl_KumpulanKerja.Where(x => x.fld_KodKumpulan.Trim() == SelectionData.Trim() && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_deleted == false && x.fld_DivisionID == DivisionID2).Select(s => s.fld_KumpulanID).FirstOrDefault();
                            var tbl_Pkjmast = dbr.tbl_Pkjmast.Where(x => x.fld_KumpulanID == KumpulanID && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_StatusApproved == 1 && x.fld_Kdaktf == "1" && x.fld_DivisionID == DivisionID2).ToList();
                            var datainpkjmast = tbl_Pkjmast.Select(s => s.fld_Nopkj.Trim()).ToList();
                            var checkatts = dbr.tbl_Kerjahdr.Join(dbr.tbl_Pkjmast, j => new { j.fld_Nopkj, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID }, k => new { k.fld_Nopkj, k.fld_NegaraID, k.fld_SyarikatID, k.fld_WilayahID, k.fld_LadangID }, (j, k) => new { j.fld_Kum, j.fld_Tarikh, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID, k.fld_Nama, j.fld_Nopkj, j.fld_Kdhdct, k.fld_DivisionID }).Where(x => datainpkjmast.Contains(x.fld_Nopkj) && x.fld_Tarikh == SelectDate && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_DivisionID == DivisionID2).ToList();
                            foreach (var checkatt in checkatts)
                            {
                                ...
                                var SalaryIncrement = dbr.tbl_PkjIncrmntSalary.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Nopkj == checkatt.fld_Nopkj && x.fld_AppStatus == true).Select(s => new { s.fld_IncrmntSalary, s.fld_DailyInsentif }).FirstOrDefault();
                                //added by faeza 17.08.2022
                                var GetLastMonthAverageSalary = dbr.tbl_GajiBulanan.Where(x => x.fld_Nopkj == checkatt.fld_Nopkj && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Month == LastSelectMonthDate.Month && x.fld_Year == LastSelectMonthDate.Year).Select(s => s.fld_PurataGaji).FirstOrDefault();
                                var GetDailyRate = db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "kadarot" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault();
                                var GetMinPay = decimal.Parse(GetDailyRate);
                                ...
                                GetLastMonthAverageSalary = GetLastMonthAverageSalary < GetMinPay ? GetMinPay : GetLastMonthAverageSalary;
                                GetLastMonthAverageSalary2 = GetLastMonthAverageSalary; //added by faeza 03.08.2023
                                //end added
                                ...
                                var GetKdRkyt = tbl_Pkjmast.Where(x => x.fld_Nopkj == checkatt.fld_Nopkj).Select(s => s.fld_Kdrkyt).FirstOrDefault();

                                if (GetKdRkyt == "MA")
                                {
                                    ActvtGLSAP = GetMapGL.Where(x => x.fld_KodAktvt == KodAktvt && x.fld_Paysheet == "PT" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_Deleted == false).Select(s => s.fld_KodGL).FirstOrDefault();
                                }
                                else
                                {
                                    ActvtGLSAP = GetMapGL.Where(x => x.fld_KodAktvt == KodAktvt && x.fld_Paysheet == "PA" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_Deleted == false).Select(s => s.fld_KodGL).FirstOrDefault();
                                }

                                //modified by faeza 17.08.2022 - add jenis bayaran n avgsalary
                                CustMod_AttWorkList.Add(new CustMod_AttWork() { Nopkj = checkatt.fld_Nopkj, Namapkj = checkatt.fld_Nama, Keteranganhdr = keteranganhdr, statushdr = statushdr, disabletextbox = tbl_JenisAktiviti.fld_DisabledFlag, Kadar = kadarharga, KadarByrn = KadarByrn, KdhByr = tbl_JenisAktiviti.fld_KdhByr, Unit = tbl_JenisAktiviti.fld_Unit, MaximumHsl = tbl_JenisAktiviti.fld_MaxProduktiviti, DailyInsentif = DailyInsentif, NegaraID = checkatt.fld_NegaraID, SyarikatID = checkatt.fld_SyarikatID, SapGL = ActvtGLSAP, jenisbayaran = jenisbayaran, avgsalary = decimal.Parse(GetLastMonthAverageSalary.ToString()), Kdhdct = checkatt.fld_Kdhdct });
                            }
                        }
                        else
                        {
                            var tbl_Pkjmast = dbr.tbl_Pkjmast.Where(x => x.fld_Nopkj == SelectionData.Trim() && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).FirstOrDefault();
                            var checkatt = dbr.tbl_Kerjahdr.Join(dbr.tbl_Pkjmast, j => new { j.fld_Nopkj, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID }, k => new { k.fld_Nopkj, k.fld_NegaraID, k.fld_SyarikatID, k.fld_WilayahID, k.fld_LadangID }, (j, k) => new { j.fld_Kum, j.fld_Tarikh, j.fld_NegaraID, j.fld_SyarikatID, j.fld_WilayahID, j.fld_LadangID, k.fld_Nama, j.fld_Nopkj, j.fld_Kdhdct, k.fld_DivisionID }).Where(x => x.fld_Nopkj == SelectionData && x.fld_Tarikh == SelectDate && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_DivisionID == DivisionID2).FirstOrDefault();
                            var SalaryIncrement = dbr.tbl_PkjIncrmntSalary.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Nopkj == checkatt.fld_Nopkj && x.fld_AppStatus == true).Select(s => new { s.fld_IncrmntSalary, s.fld_DailyInsentif }).FirstOrDefault();
                            //added by faeza 17.08.2022
                            var GetLastMonthAverageSalary = dbr.tbl_GajiBulanan.Where(x => x.fld_Nopkj == SelectionData.Trim() && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_Month == LastSelectMonthDate.Month && x.fld_Year == LastSelectMonthDate.Year).Select(s => s.fld_PurataGaji).FirstOrDefault();
                            var GetDailyRate = db.tblOptionConfigsWebs.Where(x => x.fldOptConfFlag1 == "kadarot" && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fldDeleted == false).Select(s => s.fldOptConfValue).FirstOrDefault();
                            var GetMinPay = decimal.Parse(GetDailyRate);
                            GetLastMonthAverageSalary = GetLastMonthAverageSalary == null ? GetMinPay : GetLastMonthAverageSalary;
                            GetLastMonthAverageSalary = GetLastMonthAverageSalary == 0 ? GetMinPay : GetLastMonthAverageSalary;
                            ...
                            var GetKdRkyt = tbl_Pkjmast.fld_Kdrkyt;
                        ...
                        if (!CustMod_AttWorkList.Any(a => a.SapGL == null))
                        {
                            msg = GlobalResEstate.msgWorkInfo;
                            statusmsg = "success";
                            closeform = false;
                    ...
                    //var JnisAktvtKod = db.tbl_UpahAktiviti.Where(x => x.fld_Deleted == false && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_KodJenisAktvt == JnisAktvt && x.fld_KodAktvt == KodAktvt).FirstOrDefault();
                    //JnisAktvtKod.fld_DisabledFlag - 1 = kong box & kualiti box xde, 2 = kong je xde, 3 = kong je kluar
                    bodyview = RenderRazorViewToString("WorkingDetailsForm", CustMod_AttWorkList, false, timeIn, timeOut);
                }
                else
                {
                    msg = "GL code not found.";
        ...
        public string RenderRazorViewToString(string viewname, object dataview, bool CutOfDateStatus, object checkIn, object checkOut)
        {
            ViewData.Model = dataview;
            ViewBag.CutOfDateStatus = CutOfDateStatus;
            ViewBag.checkin = checkIn;
        ...
        public JsonResult GetOTHour(string TimeIn, string TimeOut)
        {
            DateTime timeIn = DateTime.ParseExact(TimeIn, "HH:mm", CultureInfo.CurrentCulture);
            DateTime timeOut = DateTime.ParseExact(TimeOut, "HH:mm", CultureInfo.CurrentCulture);
            TimeSpan totalTimeSpan = (timeOut - timeIn).Duration();
            decimal totalWorkingHour = totalTimeSpan.Hours;
            if (totalTimeSpan.Minutes >= 30)
            {
