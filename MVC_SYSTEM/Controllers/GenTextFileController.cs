...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class GenTextFileController : Controller
    {
        ...
        public ViewResult _ewalletInsentiveindividu(int? MonthList, int? YearList, string print, string filter, string IncentiveList, string[] WorkerId)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? DivisionID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            else
            {
                var salaryData = dbview.vw_SpecialInsentive
                    .Where(x => x.fld_Year == YearList && x.fld_Month == MonthList &&
                                ...
                                x.fld_SyarikatID == SyarikatID &&
                                x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID &&
                                x.fld_DivisionID == DivisionID && x.fld_PaymentMode == "3" && x.fld_KodInsentif == IncentiveList && WorkerId.Contains(x.fld_Nopkj))
                    .OrderBy(x => x.fld_Nama);
        ...
        [HttpPost]
        public ActionResult _ewalletInsentiveindividu(int Month, int Year, string Incentive, string[] WorkerId)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            int? DivisionID = 0;
            ...
            try
            {
                ...
                var salaryData = dbview.vw_SpecialInsentive.Where(x => x.fld_Year == Year && x.fld_Month == Month && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID && x.fld_DivisionID == DivisionID && x.fld_PaymentMode == "3" && x.fld_KodInsentif == Incentive && WorkerId.Contains(x.fld_Nopkj)).OrderBy(x => x.fld_Nama).ToList();
        ...
        public JsonResult GetWorkerEwallet(int Year, int Month)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
            ...
            var maybankrcmsList = dbview.vw_PaySheetPekerja.Where(x => x.fld_LadangID == LadangID && x.fld_Year == Year && x.fld_Month == Month).Select(s => new { s.fld_Nopkj, s.fld_Nama }).OrderBy(o => o.fld_Nama).ToList();
        ...
        public JsonResult GetWorkerEwallet2(int Year, int Month)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            string host, catalog, user, pass = "";
            ...
            var maybankrcmsList = dbview.vw_SpecialInsentive.Where(x => x.fld_LadangID == LadangID && x.fld_Year == Year && x.fld_Month == Month).Select(s => new { s.fld_Nopkj, s.fld_Nama }).OrderBy(o => o.fld_Nama).ToList();
        ...
        public JsonResult CheckGenDataDetail(int Month, int Year, string[] WorkerId)
        {
           ...
           List<vw_PaySheetPekerja> PaySheetPekerjaList = new List<vw_PaySheetPekerja>();
            ...
            if (PaySheetPekerjaList.Count() != 0)
            {
                TotalGaji = PaySheetPekerjaList.Sum(s => s.fld_GajiBersih);
                CountData = PaySheetPekerjaList.Count();
                msg = GlobalResEstate.msgDataFound;
                statusmsg = "success";
            }
            else
        ...
        public JsonResult CheckGenDataDetail3(int Month, int Year, string[] WorkerId, string Incentive)
        {
            string msg = "";
            string statusmsg = "";
            ...
            List<vw_SpecialInsentive> PaySheetPekerjaList = new List<vw_SpecialInsentive>();
            if (WorkerId == null)
                WorkerId = new string[] { "0" };
            if (WorkerId.Contains("0"))
            {
                PaySheetPekerjaList = dbview.vw_SpecialInsentive.Where(x => x.fld_LadangID == LadangID && x.fld_Year == Year && x.fld_Month == Month && x.fld_KodInsentif == Incentive).ToList();
            }
            else
            {
                PaySheetPekerjaList = dbview.vw_SpecialInsentive.Where(x => x.fld_LadangID == LadangID && x.fld_Year == Year && x.fld_Month == Month && x.fld_KodInsentif == Incentive && WorkerId.Contains(x.fld_Nopkj)).ToList();
            }
            var SyarikatDetail = db.tbl_Syarikat.Where(x => x.fld_SyarikatID == SyarikatID).FirstOrDefault();
            string filename = "M2E LABOR (" + SyarikatDetail.fld_NamaPndkSyarikat.ToUpper() + ") " + "" + stringmonth + stringyear + ".txt";
